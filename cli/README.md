...
| `vaults deposit` | Deposits everything in `composer.lock`, offers to wire composer.json (public and, if needed, private) and `--write` pins the lock to Vaults. Nothing to reinstall afterwards. |
